...
$mesFiltro = isset($_GET['mes']) && $_GET['mes'] !== '' ? $_GET['mes'] : null;

if ($añoFiltro !== null && !is_numeric($añoFiltro)) {
    $añoFiltro = null; 
}

if ($mesFiltro !== null && (!is_numeric($mesFiltro) || $mesFiltro < 1 || $mesFiltro > 12)) {
    $mesFiltro = null; 
}

$ventasPorMomento = Venta::consultarVentasPorMomentoConsumo($mesFiltro, $añoFiltro);
?>
<!DOCTYPE html>
<html lang="es">
<head>
  ...
  <title>Ventas por Momento de Consumo | Colombiano, Coma Contento</title>
...
?>
  ...
  <section class="hero">
    <div class="container">
      <h1 class="display-4 fw-bold">Ventas por Momento de Consumo</h1>
      <p class="lead">Analiza el desempeño de nuestros platos por momento del día</p>
    </div>
  </section>
...
<div class="filtro-container">
  <form method="get" class="row g-3 align-items-center">
    <input type="hidden" name="pid" value="<?= htmlspecialchars(base64_encode('presentacion/VentasPorMomento.php')) ?>">
    ...
    <div class="col-md-3">
      ...
      <select name="mes" id="mes" class="form-select">
        ...
        foreach ($meses as $num => $nombre): ?>
          <option value="<?= htmlspecialchars($num) ?>" <?= ($mesFiltro == $num) ? 'selected' : '' ?>>
    ...
    <div class="col-md-4">
      ...
      <?php if($añoFiltro || $mesFiltro): ?>
        <a href="?pid=<?= htmlspecialchars(base64_encode('presentacion/VentasPorMomento.php')) ?>" class="btn btn-outline-secondary mt-md-4 ms-2">Limpiar</a>
      <?php endif; ?>
    </div>
  </form>
</div>
    ...
    <div class="card">
      <div class="card-header">
        <h3 class="mb-0">Resultados de Ventas por Momento de Consumo</h3>
      </div>
      <div class="card-body">
        <?php if (!empty($ventasPorMomento)): ?>
          <?php foreach ($ventasPorMomento as $momento): ?>
            <h4 class="momento-header">
              <?= htmlspecialchars($momento['nombre_momento']) ?>
              <span class="badge bg-warning text-dark float-end">
                Total: $<?= number_format(array_sum(array_column($momento['platos'], 'total_vendido')), 2) ?>
              </span>
            </h4>
            ...
            <div class="table-responsive">
              <table class="table table-hover">
                ...
                <tbody>
                  <?php foreach ($momento['platos'] as $venta): ?>
                    <tr>
